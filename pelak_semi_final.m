...
% % 
% % end
%figure,imshow(pbo),title('*BEDOONE ZAVIYE*') %HAZFE ZAVIY
%%
...
% for i=1:1
% for j=1:8
% temp=imread(['E:\matlabfile\New folder\' num2str(j) '.jpg']);
% temp=im2bw(temp);
% nf1=temp.*cx{1,i};
% nf2=sum(sum(nf1));
% nf(j)=nf2/(sum(sum(temp)));
...
% end
%%  ********   END   *********
%PAYANE PARDAZESH VA NAYASH PELAK DAR  " COMMAND WINDOW "